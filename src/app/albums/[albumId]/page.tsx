  ...
  const fetchAlbum = useCallback(async () => {
    try {
      ...
      setActiveCeremony((current) => {
        ...
        if (data.ceremonies?.some((c: Ceremony) => c.id === current)) return current;
        return data.ceremonies?.[0]?.id ?? null;
      });

      const validPhotoIds = new Set((data.ceremonies ?? []).flatMap((c: Ceremony) => c.photos.map((photo: Photo) => photo.id)));
  ...
  return (
    <div style={{ minHeight: "100vh", background: "var(--cream)" }}>
        ...
        <div style={{ maxWidth: 1400, margin: "0 auto", display: "flex", alignItems: "center", justifyContent: "space-between", height: 64, gap: 16 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 16, minWidth: 0 }}>
            ...
            <div style={{ minWidth: 0 }}>
              <span style={{ fontFamily: "var(--font-display)", fontSize: 18, color: "var(--espresso)", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{album.title}</span>
              <span className="hidden sm:inline" style={{ fontSize: 13, color: "var(--taupe)", marginLeft: 10 }}>{album.clientName}</span>
            </div>
          </div>
          ...
          <div className="inline-flex md:block items-center gap-2 px-4 md:px-0 py-2 md:py-0">
            {album.ceremonies.map((ceremony) => {
              ...
              return (
                <button 
                  key={ceremony.id} 
                  onClick={() => setActiveCeremony(ceremony.id)} 
                  ...
                  style={{ 
                    background: activeCeremony === ceremony.id ? "var(--warm-white)" : "transparent", 
                    borderColor: activeCeremony === ceremony.id ? "var(--gold)" : "transparent", 
                    cursor: "pointer" 
                  }}
                >
                  <span style={{ display: "flex", alignItems: "center", gap: 8, fontSize: 13, color: activeCeremony === ceremony.id ? "var(--espresso)" : "var(--brown)", fontWeight: activeCeremony === ceremony.id ? 500 : 400 }}>
                    <FolderOpen size={14} />{ceremony.name}
                  </span>
                  <div style={{ display: "flex", alignItems: "center", gap: 4 }}>
                    {finals.length > 0 ? <span style={pill("rgba(201,150,58,0.2)", "var(--gold)")}>FINALS</span> : null}
                    {selected > 0 ? <span style={pill("rgba(201,150,58,0.12)", "var(--gold)")}>S:{selected}</span> : null}
                    <span style={{ fontSize: 11, color: "var(--taupe)", background: "var(--sand)", padding: "2px 7px", borderRadius: 100 }}>{originals.length}</span>
                  </div>
                </button>
              );
            })}
            ...
            <>
              ...
              <div className="card" style={{ padding: 18, marginBottom: 24 }}>
                ...
                {settingsOpen && (
                  <div style={{ marginTop: 18, display: "grid", gap: 18 }}>
                    <div>
                      <p style={settingsLabel}>Compression Format</p>
                      <div style={{ display: "flex", gap: 8 }}>
                        {(["webp", "jpeg", "original"] as CompressionFormat[]).map((format) => (
                          <button key={format} onClick={() => setCompressionFormat(format)} style={{ padding: "10px 14px", borderRadius: 10, border: `1px solid ${compressionFormat === format ? "var(--gold)" : "var(--sand)"}`, background: compressionFormat === format ? "rgba(201,150,58,0.08)" : "var(--warm-white)", color: compressionFormat === format ? "var(--espresso)" : "var(--brown)", cursor: "pointer", fontSize: 13, fontWeight: 500 }}>{format.toUpperCase()}</button>
                        ))}
                      </div>
                    </div>
                    <label style={{ display: "grid", gap: 8 }}>
                      <span style={settingsLabel}>Quality: {compressionQuality}%</span>
                      <input type="range" min={10} max={100} value={compressionQuality} onChange={(event) => setCompressionQuality(Number(event.target.value))} />
                    </label>
                    <label style={{ display: "grid", gap: 8 }}>
                      <span style={settingsLabel}>Preparation Concurrency: {compressionConcurrency}</span>
                      <input type="range" min={1} max={4} value={compressionConcurrency} onChange={(event) => setCompressionConcurrency(Number(event.target.value))} />
                      <span style={{ fontSize: 12, color: "var(--brown)" }}>Browser-only setting. Higher values may prepare faster on strong laptops but can make weaker machines stutter.</span>
                    </label>
                    <label style={{ display: "grid", gap: 8 }}>
                      <span style={settingsLabel}>Duplicate Threshold: {dedupThreshold}</span>
                      <input type="range" min={1} max={20} value={dedupThreshold} onChange={(event) => setDedupThreshold(Number(event.target.value))} />
                      <span style={{ fontSize: 12, color: "var(--brown)" }}>Lower values are stricter. Higher values group looser visual matches.</span>
                    </label>
                    <div style={{ display: "flex", justifyContent: "space-between", gap: 12, alignItems: "center", flexWrap: "wrap" }}>
                      <p style={{ fontSize: 12, color: "var(--brown)" }}>{compressionFormat === "original" ? "New uploads keep their original file and quality." : "New uploads are compressed in the browser before they enter the queue."}</p>
                      <button className="btn-gold" onClick={saveAlbumDefaults} disabled={isSavingDefaults} style={{ fontSize: 12 }}>{isSavingDefaults ? <Loader2 size={12} style={{ animation: "spin 1s linear infinite" }} /> : <Check size={12} />}Save as album default</button>
                    </div>
                  </div>
                )}
              </div>
              ...
              {activeCeremonyData.photos.length > 0 ? (
                <div className="photo-grid">
                  {activeCeremonyData.photos.map((photo, index) => (
                    <PhotoCard 
                      key={photo.id} 
                      photo={photo} 
                      onDelete={deletePhoto} 
                      onSelect={togglePhotoSelection} 
                      onOpen={() => setLightbox({ photos: activeCeremonyData.photos, index })} 
      ...
      {selectedPhotos.length > 0 && (
        <div style={{ position: "fixed", bottom: 32, left: "50%", transform: "translateX(-50%)", background: "var(--espresso)", color: "#fff", padding: "12px 24px", borderRadius: 100, display: "flex", alignItems: "center", gap: 14, boxShadow: "0 10px 30px rgba(0,0,0,0.2)", zIndex: 100, flexWrap: "wrap", justifyContent: "center", maxWidth: "calc(100vw - 32px)" }}>
          <span style={{ fontSize: 13, fontWeight: 500 }}>{selectedPhotos.length} photo{selectedPhotos.length === 1 ? "" : "s"} selected</span>
          <div style={{ width: 1, height: 16, background: "rgba(255,255,255,0.2)" }} />
          <button onClick={() => setSelectedPhotos([])} style={{ background: "none", border: "none", color: "var(--sand)", cursor: "pointer", fontSize: 13 }}>Cancel</button>
          <button onClick={() => applyBlur(true)} disabled={isApplyingBlur} style={selectionAction("var(--gold)")}>{isApplyingBlur ? <Loader2 size={14} style={{ animation: "spin 1s linear infinite" }} /> : null}Blur Selected</button>
          <button onClick={() => applyBlur(false)} disabled={isApplyingBlur} style={selectionAction("rgba(250,247,242,0.14)")}>{isApplyingBlur ? <Loader2 size={14} style={{ animation: "spin 1s linear infinite" }} /> : null}Unblur Selected</button>
          <button onClick={deleteSelectedPhotos} disabled={isDeletingBatch} style={selectionAction("var(--blush)")}>{isDeletingBatch ? <Loader2 size={14} style={{ animation: "spin 1s linear infinite" }} /> : <Trash2 size={14} />}Delete</button>
        </div>
      )}
      ...
      {lightbox && (
        <AlbumLightbox 
          ...
          onNavigate={(direction) => setLightbox((current) => current ? { ...current, index: direction === "next" ? Math.min(current.index + 1, current.photos.length - 1) : Math.max(current.index - 1, 0) } : null)} 
          onToggleSelect={(photoId) => togglePhotoSelection(photoId, !selectedPhotos.includes(photoId))} 
        />
      )}
      ...
      {showDuplicateModal && (
        <DuplicateModal 
          ...
          onSelectDuplicates={() => { setSelectedPhotos((prev) => Array.from(new Set([...prev, ...duplicateGroups.flatMap((group) => group.duplicates.map((photo) => photo.id))]))); setShowDuplicateModal(false); }} 
        />
      )}
...
function PhotoCard({ photo, onDelete, onSelect, onOpen, isSelected, showCheckbox }: { photo: Photo; onDelete: (id: string) => void; onSelect: (id: string, selected: boolean) => void; onOpen: () => void; isSelected: boolean; showCheckbox: boolean; }) {
  ...
  return (
    <div onMouseEnter={() => setHovered(true)} onMouseLeave={() => setHovered(false)} onClick={onOpen} style={{ aspectRatio: "1", borderRadius: 10, overflow: "hidden", background: "var(--sand)", position: "relative", cursor: "pointer" }}>
      {!loaded && <div className="skeleton" style={{ position: "absolute", inset: 0 }} />}
      <img src={photo.url} alt={photo.originalName} onLoad={() => setLoaded(true)} style={{ width: "100%", height: "100%", objectFit: "cover", opacity: loaded ? 1 : 0, transition: "opacity 0.3s ease, transform 0.25s ease, filter 0.25s ease", transform: hovered ? "scale(1.02)" : "scale(1)", filter: photo.isBlurred ? "blur(12px) saturate(0)" : "none" }} />
      {checkboxVisible && (
        <div onClick={(event) => { event.stopPropagation(); onSelect(photo.id, !isSelected); }} style={{ position: "absolute", top: 8, left: 8, zIndex: 10, width: 20, height: 20, borderRadius: 4, background: isSelected ? "var(--gold)" : "rgba(255,255,255,0.86)", border: isSelected ? "none" : "2px solid var(--taupe)", display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer" }}>
          {isSelected && <Check size={14} color="#fff" strokeWidth={3} />}
        </div>
      )}
      {photo.isBlurred && (
        <div title="Blurred in album manager" style={{ position: "absolute", top: 8, left: checkboxVisible ? 36 : 8, zIndex: 10, width: 22, height: 22, borderRadius: 999, background: "rgba(26,18,8,0.78)", color: "#fff", display: "flex", alignItems: "center", justifyContent: "center", boxShadow: "0 2px 8px rgba(0,0,0,0.25)" }}>
          <Lock size={12} />
        </div>
      )}
      {photo.comments?.length ? (
        <div title={`${photo.comments.length} note(s)`} style={{ position: "absolute", top: 10, right: 10, width: 10, height: 10, borderRadius: "50%", background: "var(--gold)", border: "2px solid #fff", boxShadow: "0 2px 4px rgba(0,0,0,0.2)", zIndex: 5 }} />
      ) : null}
      {photo.isSelected ? (
        <div title="Client selected" style={{ position: "absolute", bottom: 8, right: 8, fontSize: 14, lineHeight: 1, color: "var(--gold)", textShadow: "0 1px 3px rgba(0,0,0,0.5)", zIndex: 5, pointerEvents: "none" }}>★</div>
      ) : null}
      <div style={{ position: "absolute", inset: 0, background: hovered ? "rgba(26,18,8,0.16)" : "transparent", transition: "background 0.2s ease" }} />
      <div style={{ position: "absolute", bottom: 0, left: 0, right: 0, padding: "24px 8px 8px", background: "linear-gradient(transparent, rgba(26,18,8,0.58))", opacity: hovered ? 1 : 0, transition: "opacity 0.2s" }}>
        <p style={{ fontSize: 11, color: "#fff", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", paddingRight: 24 }}>{photo.originalName}</p>
        <button onClick={(event) => { event.stopPropagation(); onDelete(photo.id); }} style={{ position: "absolute", bottom: 6, right: 6, background: "var(--blush)", border: "none", color: "#fff", borderRadius: 4, padding: 4, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center" }}>
          <Trash2 size={12} />
        </button>
      </div>
    </div>
