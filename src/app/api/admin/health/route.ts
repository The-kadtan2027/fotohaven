...
function getDbSize(): number | null {
  try {
    ...
    if (fs.existsSync(absoluteDbPath)) {
      return fs.statSync(absoluteDbPath).size;
    }
    return null;
  } catch {
    return null;
...
function getTunnelInfo(): TunnelInfo | null {
    ...
    try {
      ...
      if (cloudflaredProc) {
        ...
        return {
          ...
          status,
          publicUrl,
          detail: status === "online" ? "cloudflared is running under PM2" : "cloudflared process exists but is not online",
        };
      }
      ...
      if (tailscaleProc) {
        ...
        return {
          ...
          status,
          publicUrl,
          detail: status === "online" ? "Tailscale process is running under PM2" : "Tailscale process exists but is not online",
        };
      }
    } catch {}
  }
  ...
  if (cloudflaredProcess) {
    return { provider: "cloudflare", status: "online", publicUrl, detail: "cloudflared process detected" };
  }

  const tailscaleProcess = safeExec("pgrep -af tailscale");
  if (tailscaleProcess) {
    return { provider: "tailscale", status: "online", publicUrl, detail: "Tailscale process detected" };
  }
  ...
  if (publicUrl) {
    return {
      ...
      status: "unknown",
      publicUrl,
      detail: "Public URL configured, but tunnel process not detected",
    };
  }

  return { provider: "unknown", status: "unknown", publicUrl: null, detail: "No public tunnel URL configured" };
}
